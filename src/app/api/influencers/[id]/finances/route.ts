...
export async function GET(
  ...
  const rows = db.prepare(
    "SELECT * FROM influencer_finances WHERE influencer_id = ? ORDER BY created_at DESC"
  ).all(id);
...
export async function PATCH(
  ...
  const existing = db.prepare("SELECT * FROM influencer_finances WHERE id = ? AND influencer_id = ?").get(finance_id, id);
  ...
  db.prepare(`UPDATE influencer_finances SET ${fields.join(", ")} WHERE id = ?`).run(...values);
  const updated = db.prepare("SELECT * FROM influencer_finances WHERE id = ?").get(finance_id);
...
export async function DELETE(
  ...
  db.prepare("DELETE FROM influencer_finances WHERE id = ?").run(finance_id);
