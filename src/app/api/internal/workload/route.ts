...
export async function GET(req: NextRequest) {
  ...
  for (const emp of employees) {
    const likeName = `%${emp.name}%`;

    const orderSteps = (db.prepare(
      "SELECT COUNT(*) as c FROM order_steps WHERE assignee LIKE ? AND status NOT IN ('已完成','已停止')"
    ).get(likeName) as { c: number })?.c || 0;

    const influencerSteps = (db.prepare(
      "SELECT COUNT(*) as c FROM influencer_steps WHERE assignee LIKE ? AND status NOT IN ('已完成','已停止')"
    ).get(likeName) as { c: number })?.c || 0;

    const contractInfs = (db.prepare(
      "SELECT COUNT(*) as c FROM influencers WHERE phase = 'contract' AND id IN (SELECT influencer_id FROM influencer_steps WHERE assignee LIKE ? AND status NOT IN ('已完成','已停止'))"
    ).get(likeName) as { c: number })?.c || 0;
