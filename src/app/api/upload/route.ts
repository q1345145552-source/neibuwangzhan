...
async function ensureDir(dir: string): Promise<void> {
  ...
  console.log("[上传] 目录存在?", existsSync(dir));
  console.log("[上传] uid:", (typeof process !== "undefined" && process.getuid) ? process.getuid() : "unknown");
  
  if (!existsSync(dir)) {
    console.log("[上传] 创建目录:", dir);
    await mkdir(dir, { recursive: true });
    console.log("[上传] 目录创建成功");
  }
...
export async function POST(req: NextRequest) {
  try {
    ...
    console.log("[上传] arrayBuffer 大小:", arrayBuf.byteLength);
    const buffer = Buffer.from(arrayBuf);

    console.log("[上传] 魔数前4字节:", [...buffer.subarray(0, 8)]);
    if (!verifyMagic(buffer.subarray(0, 8), file.type)) {
      console.log("[上传] 魔数不匹配");
