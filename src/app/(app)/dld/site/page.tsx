...
export default function DldSitePage() {
  ...
  useEffect(() => {
    fetchOrders({ business_type_id: 5 }).then((data) => {
      setOrders(data.filter((o: Order) => o.sub_service_type === "site"));
      setLoading(false);
  ...
  return (
    <div className="flex flex-col gap-8">
      ...
      <div className="overflow-x-auto rounded-xl border border-[var(--border)] bg-[var(--background)]">
        ...
        {orders.length === 0 && <div className="py-12 text-center text-sm text-[var(--muted-foreground)]">暂无场地确认订单</div>}
      </div>

      <Link href="/orders/new" className="inline-flex w-fit items-center gap-1.5 rounded-lg bg-[var(--primary)] px-3 py-2 text-sm font-medium text-[var(--primary-foreground)]"><Plus className="size-3.5" />新建场地确认订单</Link>
    </div>
  );
}
