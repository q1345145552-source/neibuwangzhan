...
import { orders, statusLabels, statusClass } from "@/mock/orders";
...
const accentStyles = (hue: number) => ({
  statBar: `bg-[color-mix(in_oklch,oklch(0.55_0.16_${hue}),var(--background)_88%)]`,
...
export function BusinessLinePage({ businessKey, label, accentHue, description }: BusinessLinePageProps) {
  const businessOrders = useMemo(
    () => orders.filter((o) => o.businessLine === businessKey),
    [businessKey]
  );

  const stats = useMemo(() => {
    const total = businessOrders.length;
    const inProgress = businessOrders.filter((o) => o.status === "in_progress").length;
    const completed = businessOrders.filter((o) => o.status === "completed").length;
    const pending = businessOrders.filter((o) => o.status === "pending").length;
    const totalAmount = businessOrders.reduce((sum, o) => sum + o.amount, 0);
  ...
  return (
    <div className="flex flex-col gap-8">
      ...
      {/* Stats — horizontal bar layout, not same card grid */}
      <div className={cn("rounded-2xl border px-6 py-5 shadow-[0_2px_6px_rgba(0,0,0,0.04)]", a.accentBorder)}>
        <div className="flex flex-wrap items-end gap-6">
          ...
          <div className="flex flex-wrap gap-4 text-sm">
            <span>
              <span className="text-[var(--info)]">▲</span> 进行中 <strong className="font-mono tabular-nums">{stats.inProgress}</strong>
            </span>
            <span>
              <span className="text-[var(--success)]">●</span> 已完成 <strong className="font-mono tabular-nums">{stats.completed}</strong>
            </span>
            {stats.pending > 0 && (
              <span>
                <span className="text-[var(--warning)]">◉</span> 待处理 <strong className="font-mono tabular-nums">{stats.pending}</strong>
              </span>
            )}
          </div>
          <span className="ml-auto text-sm text-[var(--muted-foreground)]">
            总额 <strong className="font-mono tabular-nums text-[var(--foreground)]">¥{stats.totalAmount.toLocaleString()}</strong>
          </span>
        </div>
      </div>

      {/* Orders table — flat, no card bg */}
      <div className="flex flex-col gap-3">
        <div className="flex items-center justify-between">
          <h2 className="text-sm font-medium text-[var(--foreground)]">
            {stats.total > 0 ? `共 ${stats.total} 条订单` : "还没有订单"}
          </h2>
        </div>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-[var(--border)]">
                ...
                <th className="py-3 px-4 text-left text-xs font-medium text-[var(--muted-foreground)] tracking-wide max-sm:hidden">截止</th>
                ...
                >
                  <td className="py-3 px-4">
                    <Link
                      ...
                      className="font-mono text-xs font-medium text-[var(--accent-foreground)] hover:underline tabular-nums"
                    >
                      {order.id}
                    </Link>
                  </td>
                  ...
                  <td className="py-3 px-4 max-md:hidden text-[var(--foreground)]">{order.assignee}</td>
        ...
        {businessOrders.length === 0 && (
          <div className="py-16 text-center">
            <p className="text-sm text-[var(--muted-foreground)]">还没有{label}订单，去看看吧</p>
          </div>
        )}
      </div>

      {/* Quick actions — informal */}
      <div className="flex flex-wrap items-center gap-3 text-sm">
        <Link
          href="/orders/new"
          className="inline-flex shrink-0 items-center gap-1.5 rounded-lg bg-[var(--primary)] px-3 py-2 text-sm font-medium text-[var(--primary-foreground)] transition-all hover:bg-[color-mix(in_oklch,var(--primary),var(--foreground)_15%)] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--ring)]"
        >
        ...
        <Link
          href="/documents"
          className="inline-flex shrink-0 items-center gap-1.5 rounded-lg border border-[var(--border)] bg-[var(--background)] px-3 py-2 text-sm font-medium text-[var(--foreground)] transition-all hover:bg-[var(--muted)] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--ring)]"
        >
